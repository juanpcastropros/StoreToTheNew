...
namespace ContosoAdsWorker
{
    public class WorkerRole : RoleEntryPoint
    {
        ...
        private void ProcessQueueMessage(CloudQueueMessage msg)
        {
            ...
            var TypeId = msg.AsString;
            ImageDemostrable obj = GetObj(TypeId);
            //Ad ad = db.Ads.Find(adId);
            if (obj == null)
            {
                throw new Exception(String.Format("AdId {0} not found, can't create thumbnail", TypeId.ToString()));
            }

            Uri blobUri = new Uri(obj.ImageURL);
            ...
            obj.ThumbnailURL = outputBlob.Uri.ToString();
            db.SaveChanges();
            Trace.TraceInformation("Updated thumbnail URL in database: {0}", obj.ThumbnailURL);

            // Remove message from queue.
            this.imagesQueue.DeleteMessage(msg);
        }

        private ImageDemostrable GetObj(string typeId)
        {
            ImageDemostrable rt = null;
            string id = typeId.Split("||".ToCharArray(), StringSplitOptions.RemoveEmptyEntries)[1];
            string type = typeId.Split("||".ToCharArray(), StringSplitOptions.RemoveEmptyEntries)[0];
            switch(type)
            {
                case "Store":
                    rt = db.Stores.Find(Guid.Parse( id));
                    break;
                case "Product":
                    rt = db.Products.Find(Guid.Parse(id));
                    break;
                default:
                    rt = null;
                    break;
        ...
        public override bool OnStart()
        {
            ...
            db = new STTContext(dbConnString);

            // Open storage account using credentials from .cscfg file.
            var storageAccount = CloudStorageAccount.Parse
                (RoleEnvironment.GetConfigurationSettingValue("StorageConnectionString"));
            //var storageAccount = CloudStorageAccount.Parse
            //    (RoleEnvironment.GetConfigurationSettingValue("stgConnFull"));
